...
//		fprintf(errlog, "_processPacket:  %s \r\n", readBuffer);
	...
	case XPLREQUEST_REGISTERDATAREF:
	{
		...
		if (myBindings[refHandleCounter].xplaneDataRefHandle != NULL)
		{
			...
			sprintf_s(writeBuffer, XPLMAX_PACKETSIZE, ",%i,\"%s\"", refHandleCounter, myBindings[refHandleCounter].xplaneDataRefName);
	...
	case XPLREQUEST_REGISTERCOMMAND:
	{
		...
		if (myCommands[cmdHandleCounter].xplaneCommandHandle != NULL)
		{
			...
			sprintf_s(writeBuffer, XPLMAX_PACKETSIZE, ",%i,\"%s\"", cmdHandleCounter, myCommands[cmdHandleCounter].xplaneCommandName);
...
int XPLDevice::_parseString(char *outBuffer, char *inBuffer, int parameter, int maxSize)
{
	int cBeg;
	int pos = 0;
	int len;
